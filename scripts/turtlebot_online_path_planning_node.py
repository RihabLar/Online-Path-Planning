...
class OnlinePlanner:
        ...
        if (gridmap.header.stamp - self.last_map_time).to_sec() > 1:            
            ...
            if len(self.path) > 0:
                ...
                if not self.svc.check_path(total_path):
                    rospy.logwarn("Current path is invalid, replanning...")
                    self.plan()
    ...
    def get_goal(self, goal):
        ...
        if self.svc.there_is_map:
            ...
            rospy.loginfo(f"New goal point: {self.goal}")
    ...
    def plan(self):
        ...
        self.path = compute_path(self.current_pose[:2], self.goal, self.svc, 1000)
    ...
    def controller(self, event):
        ...
        tolerance = 0.2
        if len(self.path) > 0:
            if np.linalg.norm(self.current_pose[:2] - self.path[0]) < tolerance: # TODO: If current waypoint is reached with some tolerance move to the next waypoint. 
                rospy.loginfo("Reached waypoint, moving to next.")
                if len(self.path) > 1:  
                    del self.path[0]  # Only delete if there are more waypoints left
                # If it was the last waypoint in the path show a message indicating it 
                else:
                    rospy.loginfo("Final waypoint reached. Stopping the robot.")
                    self.__send_commnd__(0, 0) # Stop the robot
                    rospy.sleep(1)  
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', '/turtlebot/odom_ground_truth', '/turtlebot/kobuki/commands/velocity', np.array([-10.0, 10.0, -10.0, 10.0]), 0.15)
